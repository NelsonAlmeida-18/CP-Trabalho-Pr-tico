...
A partir da definição dada no enunciado, e aplicando a regra número 52 (Fokkinga) temos
\begin{eqnarray*}
\left\{
\begin{array}{lll}
...
%
\just\equiv{Definimos o funtor e o in da função}
%

|in = either (const 0) succ |\\
|F (split (q d) (split (r d) (c d))) = id + (split (q d) (split (r d) (c d)))|

%
\just\equiv{45}
%
|(split (q d) (split (r d) (c d))) . in= (split h (split k l) . id + (split (q d) (split (r d) (c d)))|

%
\just\equiv{9}
%
|(split (q d . in) (split (r d . in) (c d . in))) = (split h (split k l) . id + (split (q d) (split (r d) (c d)))|

%
\just\equiv{20 , def in}
%

|(split (either (q d (const 0)) (q d succ)) (split (either (r d (const 0)) (r d succ)) (either (c d (const 0)) (c d succ)))) = (split h (split k l) . id + (split (q d) (split (r d) (c d)))|

%
\just\equiv{9}
%
|(split (either (q d (const 0)) (q d succ)) (split (either (r d (const 0)) (r d succ)) (either (c d (const 0)) (c d succ)))) =|
|(split (h . id + (split (q d) (split (r d) (c d))) (split (k. id + (split (q d) (split (r d) (c d))) (l . id + (split (q d) (split (r d) (c d)))))|

%
\just\equiv{|h = either h1 h2 |, |k = either k1 k2|, |l = either l1 l2| , 22}
%

|(split (either (q d (const 0)) (q d succ)) (split (either (r d (const 0)) (r d succ)) (either (c d (const 0)) (c d succ)))) =|
|split (either (h1 . id) (h2 .(split (q d) (split (r d) (c d))))) (split (either (k1 . id) (k2 . (split (q d) (split (r d) (c d))))) (either (l1 . id) (l2 . (split (q d) (split (r d) (c d)))))))|
...
\left\{
\begin{array}{ll}
  |(either (q d (const 0)) (q d succ)) = (either (h1 . id) (h2 .(split (q d) (split (r d) (c d))))))|\\
  |(split (either (r d (const 0)) (r d succ)) (either (c d (const 0)) (c d succ)))) = ((split (either (k1 . id) (k2 . (split (q d) (split (r d) (c d))))) (either (l1 . id) (l2 . (split (q d) (split (r d) (c d)))))))|\\
\end{array}
...
|cata (split (either h1 h2) (split (either k1 k2) (either l1 l2)))|

%
\just\equiv{Substituição h1, k1, l1 da dedução acima}
%

|cata (split (either (const 0) h2) (split (either (const 0) k2) (either (const d) l2)))|

%
\just\equiv{Transformação em for loop conforme visto na aula teórica}
%

|cata (either ((const 0, (const 0, const d))) (split h2 (split k2 l2))|

%
\just\equiv{}
%

|for g d ((0,(0,d))) where g d ((x,(y,z))) = ((h2,(k2,l2)))|
...
1)
Apresentamos abaixo o diagrama do catamorfismo:
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |LTree A|
           \ar[d]_-{|both|}
...
|both = cata (in . B(both,id))|

%
\just\equiv{def in, 50}
%

|both = cata (either Leaf Fork . (both + id))| 

%
\just\equiv{22,1}
%

|both = cata(either (Leaf . both) Fork)|

%
\just\equiv{45}
%

|both . in = either (Leaf . both) Fork|

O que nos dá o gene: |either (Leaf . both) Fork|

Transformando em point wise:
